...
import { getFilmDetail, updateFilmDetail } from "@api/filmApi";
...
const {
  ...
  err,
  loading,
} = useAxios(getFilmDetail, [route.params.id], (data) => formatData(data));
...
const deformatData = (data) => {
  ...
  _data.category = data.category.filter((cat) => cat !== "").join("|");
...
const onSubmit = async () => {
  ...
  /* 发起通信 */
  const result = await updateFilmDetail(route.params.id, data);
